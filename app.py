...
st.set_page_config(page_title="Guitar Vault", layout="wide", initial_sidebar_state="expanded")
...
def init_db():
    ...
    cols = ['pickups', 'anno']
    for col in cols:
        try: c.execute(f"ALTER TABLE chitarre ADD COLUMN {col} TEXT")
        except: pass
    conn.commit()
    conn.close()
    ...
    with st.form("nuova_chitarra", clear_on_submit=True):
        ...
        if st.form_submit_button("Salva nel Vault"):
            ...
            c.execute("INSERT INTO chitarre (modello, serie, anno, marca_corde, spessore_corde, pickups, data_cambio, prossimo_cambio, foto_path) VALUES (?,?,?,?,?,?,?,?,?)",
                      (modello, serie, anno, marca, spessore, pickups, str(data_cambio), str(prossimo), foto_path))
            conn.commit()
            conn.close()
            st.rerun()
    ...
    for _, row in df.iterrows():
        ...
        with st.container(border=True):
            ...
            with col2:
                ...
                st.write(f"**Corde:** {row.get('marca_corde', '-')} | {row.get('spessore_corde', '-')}")
            ...
            with col3:
                ...
                if st.button(f"🗑️ Rimuovi chitarra", key=f"del_{row['id']}"):
                    if row['foto_path'] and os.path.exists(row['foto_path']): os.remove(row['foto_path'])
                    conn = sqlite3.connect(DB_NAME)
                    c = conn.cursor()
