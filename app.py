...
def save_data(data):
    ...
    try:
        ...
        subprocess.run(["git", "commit", "-m", "Auto-update database [skip ci]"], check=True)
...
col_stat3.metric("Controlli Urgenti (>4 mesi)", len(overdue_items), delta_color="inverse")
...
if st.session_state.show_form:
    ...
    with st.container(border=True):
        ...
        with st.form("gear_form", clear_on_submit=False):
            ...
            spec_2 = c5.text_input(l_spec2, value=v_spec2)
            ...
            v_cond = selected_gear.get("condition", "Mint")
